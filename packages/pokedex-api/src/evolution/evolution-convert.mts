  ...
  implements IZConverter<IPokeApiEvolutionChain, IZEvolution>
{
  public async convert(resource: IPokeApiEvolutionChain): Promise<IZEvolution> {
    ...
    const _convertTrigger = (details: IPokeApiEvolutionChainDetails) => {
      ...
      trigger =
        details.min_affection != null
          ...
          : trigger;
      trigger =
        details.min_beauty != null
          ? trigger.beauty(details.min_beauty)
          : trigger;
      trigger =
        details.gender != null
          ? trigger.gender(genders[details.gender - 1])
          : trigger;
      trigger =
        details.min_happiness != null
          ...
          : trigger;
      trigger =
        details.held_item != null
          ? trigger.held(details.held_item.name!)
          : trigger;
      trigger =
        details.item != null ? trigger.item(details.item.name!) : trigger;
      trigger =
        details.min_level != null ? trigger.level(details.min_level) : trigger;
      trigger =
        details.location != null
          ? trigger.location(details.location.name!)
          : trigger;
      trigger =
        details.known_move != null
          ? trigger.move(details.known_move.name!)
          : trigger;
      trigger =
        details.known_move_type != null
          ? trigger.moveType(details.known_move_type.name! as ZType)
          : trigger;
      trigger =
        details.party_species != null
          ? trigger.partySpecies(details.party_species.name!)
          : trigger;
      trigger =
        details.party_type != null
          ? trigger.partyType(details.party_type.name! as ZType)
          : trigger;
      trigger = details.time_of_day
        ? trigger.time(details.time_of_day)
        : trigger;
      trigger = details.trade_species
        ? trigger.trade(details.trade_species.name!)
        : trigger;
    ...
    const evolution = new ZEvolutionBuilder()
      .id(id)
      .head(_convert(chain))
      .build();
