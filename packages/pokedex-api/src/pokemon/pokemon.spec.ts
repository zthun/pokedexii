...
import { ZPokemonBuilder, ZTypeBuilder } from "@zthun/pokedex";
...
describe("ZPokemonApi", () => {
  ...
  beforeAll(async () => {
    const options = new ZDatabaseOptionsBuilder()
      .database("pokemon-test-database")
      .timeout(30000)
      .build();
  ...
  beforeEach(async () => {
    ...
    const _pokemon = pokemon.map((p) =>
      new ZPokeApiPokemonBuilder().from(p).build(),
    );
    const _types = [
      new ZPokeApiTypeBuilder()
        .from(new ZTypeBuilder().electric().build())
        .build(),
      new ZPokeApiTypeBuilder().from(new ZTypeBuilder().fire().build()).build(),
      new ZPokeApiTypeBuilder()
        .from(new ZTypeBuilder().flying().build())
        .build(),
      new ZPokeApiTypeBuilder().from(new ZTypeBuilder().rock().build()).build(),
      new ZPokeApiTypeBuilder()
        .from(new ZTypeBuilder().water().build())
        .build(),
    ];
