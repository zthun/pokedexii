...
export class ZTypesListService implements IZResourceListService<IZType> {
  public constructor(
    ...
    private readonly _converter: IZConverter<IPokeApiType[], IZType[]>,
    @Inject(ZSearchToken)
    private readonly _search: IZConverter<
      string | undefined,
      IZFilter | undefined
    >,
  ) {}
  ...
  public async list(request: IZDataRequest): Promise<IZPage<IZType>> {
    const truthy = new ZFilterUnaryBuilder()
      .subject("name")
      .isNotNull()
      .build();
    ...
    const types = await this._dal.read<IPokeApiType>(
